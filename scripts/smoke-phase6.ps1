...
if ($null -eq $health -or $health.status -ne 'ok') { throw "API did not become ready: $lastHealthError" }
...
if (-not $schemas) { throw 'Phase 6 schema catalog returned no content.' }
...
$rulesetCatalog = Invoke-RestMethod -Uri "$ApiBase/api/v1/rulesets" -Method Get
if ($null -eq $rulesetCatalog.count -or $null -eq $rulesetCatalog.rulesets) {
    throw "Ruleset catalog returned an invalid contract: $($rulesetCatalog | ConvertTo-Json -Depth 8)"
}
...
$payload = @{
    ...
    pinned_revisions = @{ 'tpl-smoke' = 1 }
    slot_values = @{ analyte = 'ANA'; sample_type = 'Serum' }
    objects = @(
        @{
            id = 'tpl-smoke'
            type = 'template'
            ...
            revisions = @(
                @{
                    ...
                    sentence_segments = @(
                        @{ segment_id = 'seg-smoke-1'; segment_type = 'sentence'; source_text = 'Bestimmung von {{analyte}} in {{sample_type}}.'; source_revision = 1 }
                    )
                    slots = @(
...
$result = Invoke-RestMethod -Uri "$ApiBase/api/v1/content/resolve" -Method Post -ContentType 'application/json' -Body $payload
if (-not $result.blocks -or $result.blocks.Count -lt 1) { throw 'Resolver returned no blocks.' }
$rendered = $result.blocks[0].rendered_content
if ($rendered -notmatch 'ANA' -or $rendered -notmatch 'Serum') { throw "Resolver did not render expected slot values: $rendered" }
if ([string]::IsNullOrWhiteSpace($result.checksum)) { throw 'Resolver result has no checksum.' }
$fatal = @($result.findings | Where-Object { $_.severity -in @('ERROR', 'FATAL') })
if ($fatal.Count -gt 0) { throw "Resolver returned blocking findings: $($fatal | ConvertTo-Json -Depth 8)" }
...
$createdDecision = Invoke-RestMethod -Uri "$ApiBase/api/v1/reviews/migrations/$migrationId/decisions" -Method Post -ContentType 'application/json' -Body $reviewBody
...
$reviewHistory = Invoke-RestMethod -Uri "$ApiBase/api/v1/reviews/migrations/$migrationId/decisions" -Method Get
