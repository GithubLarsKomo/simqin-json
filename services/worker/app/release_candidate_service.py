...
def _validate_trusted_sources(objects: dict[str, ContentObject], resolved_tree: Any) -> None:
    ...
    for object_id, revision in resolved_pairs:
        ...
        if source_revision is None:
            findings.append({"code": "canonical-source-revision-missing", "message": f"Resolved canonical revision {object_id}@{revision} is unavailable", "object_id": object_id, "revision": revision})
            continue
        try:
            trusted = store.get(object_id, revision)
        except ValueError as exc:
            findings.append({"code": "canonical-source-integrity-failed", "message": str(exc), "object_id": object_id, "revision": revision})
            continue
        if trusted is None:
            findings.append({"code": "canonical-source-not-registered", "message": f"Trusted canonical source {object_id}@{revision} is not registered", "object_id": object_id, "revision": revision})
            continue
        if not revision_matches_snapshot(source_revision, trusted["revision_payload"]):
            findings.append({"code": "canonical-source-mismatch", "message": f"Candidate content {object_id}@{revision} differs from its trusted canonical source snapshot", "object_id": object_id, "revision": revision})
    if findings:
        raise ReleaseBuildError(findings)


def _trusted_configuration_catalog(parameter_rows: list[dict[str, Any]], values: list[ConfigurationValue]) -> tuple[ConfigurationCatalog, list[str]]:
    ...
    for parameter_id, revision in sorted(required):
        if not parameter_id or revision < 1:
            findings.append({"code": "configuration-parameter-reference-invalid", "message": f"Invalid configuration parameter reference {parameter_id}@{revision}", "parameter_id": parameter_id, "revision": revision})
            continue
        try:
            trusted = store.get(parameter_id, revision)
        except ValueError as exc:
            findings.append({"code": "configuration-parameter-integrity-failed", "message": str(exc), "parameter_id": parameter_id, "revision": revision})
            continue
        if trusted is None:
            findings.append({"code": "configuration-parameter-not-registered", "message": f"Trusted configuration parameter {parameter_id}@{revision} is not registered", "parameter_id": parameter_id, "revision": revision})
            continue
        ...
        if asserted is not None and asserted.to_dict() != trusted_parameter.to_dict():
            findings.append({"code": "configuration-parameter-mismatch", "message": f"Candidate parameter {parameter_id}@{revision} differs from the trusted configuration catalog", "parameter_id": parameter_id, "revision": revision})
            continue
...
def _trusted_ruleset(ruleset_revision: str, asserted_rows: list[dict[str, Any]]) -> tuple[list[MultiplicityRule], str]:
    revision = ruleset_revision.strip()
    if not revision:
        if asserted_rows:
            raise ReleaseBuildError([{
                "code": "ruleset-revision-required",
                "message": "ruleset_revision is required when multiplicity rules are supplied",
            }])
        return [], ""
    try:
        trusted = RulesetStore().get(revision)
    except ValueError as exc:
        raise ReleaseBuildError([{
            "code": "ruleset-integrity-failed",
            "message": str(exc),
            "ruleset_revision": revision,
        }]) from exc
    if trusted is None:
        raise ReleaseBuildError([{
            "code": "ruleset-not-registered",
            "message": f"Trusted ruleset {revision} is not registered",
            "ruleset_revision": revision,
        }])
    trusted_rows = list(trusted.get("rules", []))
    if asserted_rows:
        asserted = sorted((MultiplicityRule.from_dict(row).to_dict() for row in asserted_rows), key=lambda row: row["object_id"])
        if asserted != trusted_rows:
            raise ReleaseBuildError([{
                "code": "ruleset-mismatch",
                "message": f"Candidate multiplicity rules differ from trusted ruleset {revision}",
                "ruleset_revision": revision,
            }])
...
def build_from_candidate_payload(payload: dict[str, Any], *, release_id: str, version: int, created_by: str, candidate_id: str = "", candidate_checksum: str = "") -> Any:
    ...
    catalog, configuration_checksums = _trusted_configuration_catalog(list(payload.get("configuration_parameters", [])), values)
...
def validate_candidate_payload(payload: dict[str, Any]) -> dict[str, Any]:
    probe = build_from_candidate_payload(payload, release_id="candidate-validation", version=1, created_by="candidate-validator")
    return {
        "valid": True,
